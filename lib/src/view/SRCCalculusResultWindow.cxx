...
SRCCalculusResultWindow::SRCCalculusResultWindow(SensitivityAnalysisItem * item)
  : OTguiSubWindow(item)
  , result_(dynamic_cast<SRCCalculus*>(&*item->getCalculus().getImplementation())->getResult())
{
...
void SRCCalculusResultWindow::buildInterface()
{
...
void SRCCalculusResultWindow::outputChanged(int index)
{
  updateTable(index);
  updateBarChart();
}
...
void SRCCalculusResultWindow::updateBarChart()
{
  Q_ASSERT(barChart_);
...
void SRCCalculusResultWindow::updateTable(int index)
{
  int indexOutput = outputsComboBox_->currentIndex();
