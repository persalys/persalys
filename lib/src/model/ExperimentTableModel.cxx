  ...
  else
  {
    if (role == Qt::DisplayRole || role == Qt::EditRole)
    {
      ...
      switch (index.column())
      {
        ...
        case 2:
          return QString::number(designOfExperiment_.getValues()[indexInput], 'g', StudyTreeViewModel::DEFAULT_SIGNIFICANT_DIGITS);
        case 3:
          return QString::number(designOfExperiment_.getLowerBounds()[indexInput], 'g', StudyTreeViewModel::DEFAULT_SIGNIFICANT_DIGITS);
        case 4:
          return QString::number(designOfExperiment_.getUpperBounds()[indexInput], 'g', StudyTreeViewModel::DEFAULT_SIGNIFICANT_DIGITS);
        case 5:
        {
          if (designOfExperiment_.getTypeDesignOfExperiment() == GridDesignOfExperiment::FromBoundsAndLevels)
            return QString::number(int(designOfExperiment_.getLevels()[indexInput]));
          else
            return QString::number(designOfExperiment_.getDeltas()[indexInput], 'g', StudyTreeViewModel::DEFAULT_SIGNIFICANT_DIGITS);
        }
        default:
          return QVariant();
      }
