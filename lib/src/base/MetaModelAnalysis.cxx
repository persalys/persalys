...
void MetaModelAnalysis::buildMetaModel(MetaModelAnalysisResult& result, const Function& function)
{
  ...
  if (designOfExperiment_.hasPhysicalModel())
  {
    ...
    const PhysicalModel physicalModel = designOfExperiment_.getPhysicalModel();
    // inputs
    for(UnsignedInteger i = 0; i < inputsNames.getSize(); ++i)
    {
      if (physicalModel.hasInputNamed(inputsNames[i]))
        metaModel.addInput(physicalModel.getInputByName(inputsNames[i]));
      else
        throw InvalidArgumentException(HERE) << "The input " << inputsNames[i] << " of the metamodel is not an input of the physical model.";
    }

    //outputs
    for(UnsignedInteger i = 0; i < outputsNames.getSize(); ++i)
    {
      if (physicalModel.hasOutputNamed(outputsNames[i]))
      {
        ...
        throw InvalidArgumentException(HERE) << "The output " << outputsNames[i] << " of the metamodel is not an output of the physical model.";
    }

    // copula
    metaModel.setBlockIndependentCopula(BlockIndependentCopula(physicalModel.getCopulaCollection()));
  }
  else
  {
    // inputs
    ...
    if (isDistributionComputed_ && distribution_.getDimension() == inputsNames.getSize())
    {
      ...
      metaModel.addCopula(inputsNames, distribution_.getCopula());
    }
  }
