...
private slots:
  void TestGridMeshModel() const
  {
    ...
    QVERIFY2(wizard.errorWidget_->toPlainText().isEmpty(), "Label must be empty");
    ...
    QVERIFY2(!wizard.errorWidget_->toPlainText().isEmpty(), "Label must be not empty");
  ...
  void TestImportedMeshModel() const
  {
    ...
    QVERIFY2(wizard.validateCurrentPage(), "Page must be valid");
    QVERIFY2(sampleWidget->errorWidget_->toPlainText().isEmpty(), "Label must be empty");

    OT::Sample(1, OT::Point(1, 2.5)).exportToCSVFile(file.fileName().toStdString());
    sampleWidget->setData(file.fileName());
    QVERIFY2(!wizard.validateCurrentPage(), "Page must be not valid");
    QVERIFY2(!sampleWidget->errorWidget_->toPlainText().isEmpty(), "Label must be not empty");
