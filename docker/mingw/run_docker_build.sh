...
CXXFLAGS="-Wall -D_GLIBCXX_ASSERTIONS" ${ARCH}-w64-mingw32-cmake -DUSE_SPHINX=OFF -DUSE_SALOME=OFF \
  ...
  -DUSE_COTIRE=ON -DCOTIRE_MAXIMUM_NUMBER_OF_UNITY_INCLUDES="-j8" ../persalys
make install
...
VERSION=`cat ../persalys/VERSION`
cp /tmp/persalys/distro/windows/* .
...
then
  sudo cp persalys*.exe /tmp/persalys
  sudo chown ${uid}:${gid} /tmp/persalys/persalys*.exe
fi
